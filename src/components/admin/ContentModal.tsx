  ...
  return (
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-[rgba(0,0,0,0.6)] p-3 md:p-10">
      <div className="flex max-h-[92vh] w-full max-w-[620px] flex-col gap-[18px] overflow-y-auto rounded-[18px] border border-line10 bg-modal p-5 md:max-h-[88vh] md:p-[30px]">
        ...
        {/* 주소 칸은 토글과 무관하게 항상 보인다. 주소를 넣으면 자동으로 "포스터 사용"이 켜진다. */}
        <ImageUrlField
          label="카드 썸네일 · 세로 포스터 (사이트에 바로 반영돼요)"
          hint="영화 포스터처럼 세로 2:3 비율을 권장해요. TMDB(image.tmdb.org)처럼 배포가 허용된 이미지 주소를 쓰세요."
          value={draft.posterUrl}
          onChange={(url) => patch({ posterUrl: url, poster: url ? true : draft.poster })}
        />
        <ImageUrlField
          label="상세·히어로 배경 이미지 (선택)"
          hint="가로로 넓은 스틸컷 주소를 넣는 자리예요. 비워두면 세로 포스터를 흐리게 깔아서 자연스럽게 처리해요."
